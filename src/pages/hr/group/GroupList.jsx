...
const { Link, LoginBanner, AddButton, useNavigate, useDispatch, useSelector, groupListTodo, Loader, Search, PDF, Excel, Pagination, DataTable, GroupAction, ToastContainer, parseLinkHeader, lang } = GroupImport
...
export default function GroupList() {
    ...
    const columnDefs = [
        ...
        {
            headerName: lang.table_group_type,
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(groupListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => group_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(groupListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => group_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(groupListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => group_res(res.payload))
    }, [bool])
                ...
                <div className="col-md-12">
                    ...
                    <div className='layout d-flex justify-content-between align-items-end'>
                        ...
                        <div className='d-flex align-items-center'>
                            ...
                            {permission.some(val => val.actionName == 'Add Group') ?
                                <Link to='/add-group'>
