...
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, RegionAction, regionListTodo, lang } = RegionImport
...
export default function RegionList() {
    ...
    const region = useSelector(state => state && state.regionList && state.regionList.data)
    // console.log(region)
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(regionListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => region_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(regionListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => region_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(regionListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => region_res(res.payload))
    }, [bool])
                    ...
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            ...
                            <div className='d-flex align-items-center'>
                                ...
                                {permission.some(val => val.actionName == 'Add Region') ?
                                    <Link to='/add-region'>
