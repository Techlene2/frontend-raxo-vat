...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, regionListTodo, lang } = RegionImport
...
export default function RegionFilter(props) {
  ...
  const apply_filter = () => {
    ...
    setCurrentPage(0)
    dispatch(regionListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => region_res(res.payload))
  }
    ...
    <>
      <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
        <Offcanvas.Header closeButton>
          <Offcanvas.Title style={{ color: '#21263c' }}> {lang.region + ' ' + lang.filter}</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
