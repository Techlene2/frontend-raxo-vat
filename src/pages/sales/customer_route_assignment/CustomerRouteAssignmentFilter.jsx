...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, customerRouteAssignmentListTodo, lang } = CustomerRouteAssignmentImport

export default function CustomerRouteAssignmentFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(customerRouteAssignmentListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => customer_route_assign_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}> {lang.customer_route_assignment + ' ' + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
