...
import CustomerGroupFilter from './CustomerGroupFilter.jsx'
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, CustomerGroupAction, customerGroupListTodo, lang } = CustomerGroupImport
...
export default function CustomerGroupList() {
  ...
  const customer_group = useSelector(state => state && state.customerGroupList && state.customerGroupList.data)
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: CustomerGroupAction,
      cellRendererParams: {
        bool: bool,
        setBool: setBool,
  ...
  const perPage = (val) => {
    ...
    setPageSize(val)
    setCurrentPage(0)
    dispatch(customerGroupListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => customer_group_res(res.payload))
  }

  const prev = (page, size) => {
    setTableLoading(true)
    setCurrentPage(currentPage - 1)
    dispatch(customerGroupListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => customer_group_res(res.payload))
  }
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(customerGroupListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => customer_group_res(res.payload))
  }, [bool])
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className='layout d-flex justify-content-between align-items-end'>
              <h6>{lang.customer_group}</h6>
              <div className='d-flex align-items-center'>

                {permission.some(val => val.actionName == 'Add Customer Group') ?
