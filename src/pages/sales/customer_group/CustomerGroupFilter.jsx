...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, customerGroupListTodo, lang } = CustomerGroupImport

export default function CustomerGroupFilter(props) {
  ...
  const apply_filter = () => {
    ...
    setCurrentPage(0)
    dispatch(customerGroupListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => customer_group_res(res.payload))
  }
    ...
    <>
      <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
        <Offcanvas.Header closeButton>
          <Offcanvas.Title style={{ color: '#21263c' }}> {lang.customer_group + ' ' + lang.filter}</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
