...
const { Link, AddButton, Search, PDF, Excel, useNavigate, useDispatch, useSelector, DataTable, Pagination, parseLinkHeader, Loader, ToastContainer, routesListTodo, RoutesAction, lang } = RoutesImport
...
export default function RoutesList() {
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(routesListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => routes_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(routesListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => routes_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(routesListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => routes_res(res.payload))
    }, [bool])
                    ...
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            ...
                            <div className='d-flex align-items-center'>
                                ...
                                {permission.some(val => val.actionName == 'Add Route') ?
                                    <Link to='/add-routes'>
