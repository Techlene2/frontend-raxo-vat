...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, dispatchPlanningListTodo, lang } = DispatchPlanningImport

export default function DispatchPlanningFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(dispatchPlanningListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => planning_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}> {lang.dispatch_planning + ' ' + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
