...
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, GradeAction, gradeListTodo, lang } = GradeImport
...
export default function GradeList() {
    ...
    const grade = useSelector(state => state && state.gradeList && state.gradeList.data)
    // console.log(grade)
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(gradeListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => grade_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(gradeListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => grade_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(gradeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => grade_res(res.payload))
    }, [bool])
                    ...
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            ...
                            <div className='d-flex align-items-center'>
                                ...
                                {permission.some(val => val.actionName == 'Add Grade') ?
                                    <Link to='/add-grade'>
