...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, customerCategoryListTodo, lang } = CustomerCategoryImport

export default function CustomerCategoryFilter(props) {
  ...
  const apply_filter = () => {
    ...
    setCurrentPage(0)
    dispatch(customerCategoryListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => customer_category_res(res.payload))
  }
    ...
    <>
      <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
        <Offcanvas.Header closeButton>
          <Offcanvas.Title style={{ color: '#21263c' }}> {lang.customer_category + ' ' + lang.filter}</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
