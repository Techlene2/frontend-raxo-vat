...
import CustomerCategoryFilter from './CustomerCategoryFilter.jsx'
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, CustomerCategoryAction, customerCategoryListTodo, lang } = CustomerCategoryImport
...
export default function CustomerCategoryList() {
  ...
  const customer_category = useSelector(state => state && state.customerCategoryList && state.customerCategoryList.data)
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: CustomerCategoryAction,
      cellRendererParams: {
        bool: bool,
        setBool: setBool,
  ...
  const perPage = (val) => {
    ...
    setPageSize(val)
    setCurrentPage(0)
    dispatch(customerCategoryListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => customer_category_res(res.payload))
  }

  const prev = (page, size) => {
    setTableLoading(true)
    setCurrentPage(currentPage - 1)
    dispatch(customerCategoryListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => customer_category_res(res.payload))
  }
  ...
  const customer_category_res = (res) => {
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(customerCategoryListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => customer_category_res(res.payload))
  }, [bool])
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className='layout d-flex justify-content-between align-items-end'>
              <h6>{lang.customer_category}</h6>
              <div className='d-flex align-items-center'>

                {permission.some(val => val.actionName == 'Add Customer Category') ?
                  <Link to='/add-customer-category'>
