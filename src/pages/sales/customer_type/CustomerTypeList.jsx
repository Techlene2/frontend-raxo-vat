...
import CustomerTypeFilter from './CustomerTypeFilter.jsx'
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, CustomerTypeAction, customerTypeListTodo, lang } = CustomerTypeImport
...
export default function CustomerTypeList() {
  ...
  const customer_type = useSelector(state => state && state.customerTypeList && state.customerTypeList.data)
  ...
  const columnDefs = [
    ...
    },
    {
      ...
      cellRenderer: CustomerTypeAction,
      cellRendererParams: {
        bool: bool,
        setBool: setBool,
  ...
  const perPage = (val) => {
    ...
    setPageSize(val)
    setCurrentPage(0)
    dispatch(customerTypeListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => customer_type_res(res.payload))
  }

  const prev = (page, size) => {
    setTableLoading(true)
    setCurrentPage(currentPage - 1)
    dispatch(customerTypeListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => customer_type_res(res.payload))
  }
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(customerTypeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => customer_type_res(res.payload))
  }, [bool])
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className='layout d-flex justify-content-between align-items-end'>
              <h6>{lang.customer_type}</h6>
              <div className='d-flex align-items-center'>

                {permission.some(val => val.actionName == 'Add Customer Type') ?
                  <Link to='/add-customer-type'>
