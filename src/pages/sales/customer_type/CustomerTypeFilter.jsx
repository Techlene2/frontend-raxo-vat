...
const { Offcanvas, Form, Button, useNavigate, useDispatch, parseLinkHeader, customerTypeListTodo, lang } = CustomerTypeImport

export default function CustomerTypeFilter(props) {
  ...
  const apply_filter = () => {
    ...
    setCurrentPage(0)
    dispatch(customerTypeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => customer_type_res(res.payload))
  }
    ...
    <>
      <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
        <Offcanvas.Header closeButton>
          <Offcanvas.Title style={{ color: '#21263c' }}> {lang.customer_type + ' ' + lang.filter}</Offcanvas.Title>
        </Offcanvas.Header>
        <Offcanvas.Body>
