...
const { Link, LoginBanner, AddButton, useNavigate, useDispatch, useSelector, parseLinkHeader, Search, PDF, Excel, Loader, DataTable, Pagination, ToastContainer, userRoleListTodo, UserRoleAction, lang } = UserRoleImport
...
export default function UserRoleList() {
    ...
    const user_role = useSelector(state => state.userRoleList && state.userRoleList.data)
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(userRoleListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => user_role_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(userRoleListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => user_role_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(userRoleListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => user_role_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.user_role}</h6>

                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Add User Role') ?
                                    <Link to='/add-user-role'>
