...
const { Link, LoginBanner, AddButton, Search, PDF, Excel, Pagination, DataTable, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, companyListTodo, CompanyAction, lang } = CompanyImport
...
export default function CompanyList() {
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(companyListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => company_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(companyListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => company_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(companyListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => company_res(res.payload))
    }, [bool])
                    ...
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            ...
                            <div className='d-flex align-items-center'>
                                ...
                                {permission.some(val => val.actionName == 'Add Company') ?
                                    <Link to='/add-company'>
