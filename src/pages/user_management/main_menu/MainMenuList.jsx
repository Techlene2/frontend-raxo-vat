...
const { Link, LoginBanner, AddButton, useNavigate, useDispatch, useSelector, menuListTodo, Loader, Search, PDF, Excel, Pagination, DataTable, MenuAction, ToastContainer, parseLinkHeader, lang } = MainMenuImport
...
export default function MainMenuList() {
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(menuListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => menu_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(menuListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => menu_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(menuListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => menu_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.main_menu}</h6>

                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Add Main Menu') ?
                                    <Link to='/create-main-menu'>
