...
const { Link, LoginBanner, AddButton, DataTable, CountryAction, Search, PDF, Excel, Pagination, useDispatch, useSelector, Loader, useNavigate, countryListTodo, ToastContainer, parseLinkHeader, lang } = CountryImport
...
export default function CountryList() {
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(countryListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => country_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(countryListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => country_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(countryListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => country_res(res.payload))
    }, [bool])
