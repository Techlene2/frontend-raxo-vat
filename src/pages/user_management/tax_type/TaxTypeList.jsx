...
import TaxTypeFilter from './TaxTypeFilter'
const { Link, LoginBanner, AddButton, useNavigate, useDispatch, useSelector, taxTypeListTodo, Loader, Search, PDF, Excel, Pagination, DataTable, TaxTypeAction, ToastContainer, parseLinkHeader, lang } = TaxTypeImport
...
export default function TaxTypeList() {
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(taxTypeListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => tax_type_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(taxTypeListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => tax_type_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(taxTypeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => tax_type_res(res.payload))
    }, [bool])
                ...
                <div className="col-md-12">
                    ...
                    <div className='layout d-flex justify-content-between align-items-end'>
                        ...
                        <div className='d-flex align-items-center'>
                            ...
                            {permission.some(val => val.actionName == 'Add Tax Type') ?
                                <Link to='/add-tax-type'>
