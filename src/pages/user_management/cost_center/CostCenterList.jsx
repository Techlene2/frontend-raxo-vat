...
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, costCenterListTodo, CostCenterAction, lang } = CostCenterImport
...
export default function CostCenterList() {
  ...
  const perPage = (val) => {
    ...
    setPageSize(val)
    setCurrentPage(0)
    dispatch(costCenterListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => cost_center_res(res.payload))
  }

  const prev = (page, size) => {
    setTableLoading(true)
    setCurrentPage(currentPage - 1)
    dispatch(costCenterListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => cost_center_res(res.payload))
  }
  ...
  useEffect(() => {
    setLoading(true)
    dispatch(costCenterListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => cost_center_res(res.payload))
  }, [bool])
        ...
        <div className="row">
          <div className="col-md-12">
            ...
            <div className='layout d-flex justify-content-between align-items-end'>
              <h6>{lang.cost_center}</h6>
              <div className='d-flex align-items-center'>

                {permission.some(val => val.actionName == 'Add Cost Center') ?
                  <Link to='/add-cost-center'>
