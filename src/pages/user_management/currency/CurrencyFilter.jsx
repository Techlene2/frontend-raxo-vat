...
const { Offcanvas, Form, Button, useNavigate, useDispatch, currencyListTodo, parseLinkHeader, lang } = CurrencyImport
...
export default function CurrencyFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(currencyListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => currency_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}>{lang.currency + ' ' + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
