...
const { Offcanvas, Form, Button, useNavigate, useDispatch, subMenuTodo, parseLinkHeader, lang } = SubMenuImport

export default function SubMenuFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(subMenuTodo({ 'method': 'GET', 'values': '', 'id': '', 'mainMenuId': '', 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => sub_menu_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}> {lang.sub_menu + ' ' + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
