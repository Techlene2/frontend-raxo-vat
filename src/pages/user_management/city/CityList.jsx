...
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, useDispatch, useSelector, Loader, useNavigate, CityAction, cityListTodo, ToastContainer, parseLinkHeader, lang } = CityImport
...
export default function CityList() {
    ...
    const city = useSelector(state => state && state.cityList && state.cityList.data)
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(cityListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => city_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(cityListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => city_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(cityListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => city_res(res.payload))
    }, [bool])
