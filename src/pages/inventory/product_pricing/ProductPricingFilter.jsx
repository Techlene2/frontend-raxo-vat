...
const { Offcanvas, Form, Button, useNavigate, useDispatch, productPricingListTodo, parseLinkHeader, Flatpickr, lang } = ProductPricingImport

export default function ProductPricingFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(productPricingListTodo({ 'pageSize': pageSize, 'pageNo': 0, 'search': search })).then((res) => product_price_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}>{lang.product_pricing + ' ' + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
