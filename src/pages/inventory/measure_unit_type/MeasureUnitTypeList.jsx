...
import MeasureUnitTypeFilter from './MeasureUnitTypeFilter'
const { Link, LoginBanner, AddButton, useNavigate, useDispatch, useSelector, measureUnitTypeListTodo, Search, PDF, Excel, Pagination, DataTable, Loader, MeasureUnitTypeAction, ToastContainer, parseLinkHeader, lang } = MeasureUnitTypeImport

export default function MeasureUnitTypeList() {
    ...
    const columnDefs = [
        ...
        },
        {
            ...
            cellRenderer: MeasureUnitTypeAction,
            cellRendererParams: {
                bool: bool,
                setBool: setBool,
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(measureUnitTypeListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => measure_unit_type_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(measureUnitTypeListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => measure_unit_type_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(measureUnitTypeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => measure_unit_type_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.measurement_unit_type}</h6>

                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Add Measure Unit Type') ?
                                    <Link to='/add-measure-unit-type'>
