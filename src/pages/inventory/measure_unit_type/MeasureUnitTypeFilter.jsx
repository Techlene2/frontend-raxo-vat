...
const { Offcanvas, Form, Button, useNavigate, useDispatch, measureUnitTypeListTodo, parseLinkHeader, lang } = MeasureUnitTypeImport

export default function MeasureUnitTypeFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(measureUnitTypeListTodo({ 'search': `pageSize=${pageSize}&pageNo=${0}&search=${search}` })).then((res) => measure_unit_type_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}>{lang.measurement_unit_type + " " + lang.filter}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
