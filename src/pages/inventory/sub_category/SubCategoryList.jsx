...
const { Link, LoginBanner, AddButton, DataTable, Search, PDF, Excel, Pagination, ToastContainer, Loader, useNavigate, useDispatch, useSelector, parseLinkHeader, subCategoryListTodo, SubCategoryAction, lang } = SubCategoryImport
...
export default function SubCategoryList() {
    ...
    const columnDefs = [
        ...
        {
            headerName: lang.table_category,
            field: 'category.name',
            sortable: false,
        },
        {
        ...
        },
        {
            ...
            cellRenderer: SubCategoryAction,
            cellRendererParams: {
                bool: bool,
                setBool: setBool,
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(subCategoryListTodo({ 'search': `pageSize=${val}&pageNo=${0}` })).then((res) => sub_cat_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(subCategoryListTodo({ 'search': `pageSize=${size}&pageNo=${page}` })).then((res) => sub_cat_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(subCategoryListTodo({ 'search': `pageSize=${pageSize}&pageNo=${currentPage}` })).then((res) => sub_cat_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.sub_category}</h6>
                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Add Sub Category') ?
