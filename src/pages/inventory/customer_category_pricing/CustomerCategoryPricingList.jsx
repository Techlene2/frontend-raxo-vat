...
import CustomerCategoryPricingFilter from './CustomerCategoryPricingFilter'
const { Link, AddButton, LoginBanner, useNavigate, useDispatch, useSelector, Loader, Search, PDF, Excel, Pagination, DataTable, ToastContainer, parseLinkHeader, customerCategoryPricingListTodo, moment, lang } = CustomerCategoryPricingImport

export default function CustomerCategoryPricingList() {
    ...
    const price_list = useSelector(state => state.customerCategoryPricingList && state.customerCategoryPricingList.data)
    ...
    const columnDefs = [
        ...
        {
            headerName: lang.table_customer_cat,
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(customerCategoryPricingListTodo({ 'pageSize': val, 'pageNo': 0, 'search': search })).then((res) => product_price_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(customerCategoryPricingListTodo({ 'pageSize': size, 'pageNo': page, 'search': search })).then((res) => product_price_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(customerCategoryPricingListTodo({ 'pageSize': pageSize, 'pageNo': currentPage, 'search': search })).then((res) => product_price_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.customer_cat_pricing}</h6>

                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Update Customer Category Pricing') ?
                                    <Link to='/update-customer-category-pricing'>
