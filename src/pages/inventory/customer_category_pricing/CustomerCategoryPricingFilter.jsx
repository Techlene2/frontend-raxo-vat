...
const { Offcanvas, Form, Button, useNavigate, useDispatch, customerCategoryPricingListTodo, parseLinkHeader, Flatpickr, lang } = CustomerCategoryPricingImport

export default function CustomerCategoryPricingFilter(props) {
    ...
    const apply_filter = () => {
        ...
        setCurrentPage(0)
        dispatch(customerCategoryPricingListTodo({ 'pageSize': pageSize, 'pageNo': 0, 'search': search })).then((res) => product_price_res(res.payload))
    }
        ...
        <>
            <Offcanvas show={filter} onHide={handleClose} placement={localStorage.getItem('lang_key') == 'ar' ? 'start' : 'end'} backdrop="static" >
                <Offcanvas.Header closeButton>
                    <Offcanvas.Title style={{ color: '#21263c' }}>{'Customer Category Pricing Filter'}</Offcanvas.Title>
                </Offcanvas.Header>
                <Offcanvas.Body>
