...
import CustomerPricingFilter from './CustomerPricingFilter'
const { Link, AddButton, LoginBanner, useNavigate, useDispatch, useSelector, Loader, Search, PDF, Excel, Pagination, DataTable, ToastContainer, parseLinkHeader, customerPricingListTodo, moment, lang } = CustomerPricingImport

export default function CustomerPricingList() {
    ...
    const price_list = useSelector(state => state.customerPricingList && state.customerPricingList.data)
    ...
    const perPage = (val) => {
        ...
        setPageSize(val)
        setCurrentPage(0)
        dispatch(customerPricingListTodo({ 'pageSize': val, 'pageNo': 0, 'search': search })).then((res) => product_price_res(res.payload))
    }

    const prev = (page, size) => {
        setTableLoading(true)
        setCurrentPage(currentPage - 1)
        dispatch(customerPricingListTodo({ 'pageSize': size, 'pageNo': page, 'search': search })).then((res) => product_price_res(res.payload))
    }
    ...
    useEffect(() => {
        setLoading(true)
        dispatch(customerPricingListTodo({ 'pageSize': pageSize, 'pageNo': currentPage, 'search': search })).then((res) => product_price_res(res.payload))
    }, [bool])
                ...
                <div className="row">
                    <div className="col-md-12">
                        ...
                        <div className='layout d-flex justify-content-between align-items-end'>
                            <h6>{lang.customer_pricing}</h6>

                            <div className='d-flex align-items-center'>

                                {permission.some(val => val.actionName == 'Update Customer Pricing') ?
                                    <Link to='/update-customer-pricing'>
