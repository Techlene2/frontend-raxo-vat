...
const { Form, useFormik, Text, TextArea, SaveButton, SelectStatus, DepartmentSchema, addDepartmentTodo, useDispatch, useNavigate, ToastContainer, toast, lang } = DepartmentImport
...
export default function AddDepartmentForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(addDepartmentTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.department + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
