...
const { Form, useFormik, Text, TextArea, SelectStatus, SaveButton, TaxTypeSchema, addTaxTypeTodo, useDispatch, useNavigate, ToastContainer, toast, lang } = TaxTypeImport

export default function AddTaxTypeForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(addTaxTypeTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.tax_type + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
