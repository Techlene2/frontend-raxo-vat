...
const { Form, Text, TextArea, SelectStatus, SaveButton, useFormik, RoleSchema, addRoleTodo, useDispatch, useNavigate, toast, ToastContainer, lang } = RoleImport
...
export default function AddRoleForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(addRoleTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.role + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
                navigate('/role-list')
            }, 1500);
