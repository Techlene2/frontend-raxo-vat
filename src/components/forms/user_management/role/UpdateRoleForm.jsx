...
const { Form, Text, TextArea, SelectStatus, SaveButton, useFormik, RoleSchema, updateRoleTodo, roleDetailsTodo, Loader, useDispatch, useSelector, useLocation, useNavigate, toast, ToastContainer, lang } = RoleImport
...
export default function UpdateRoleForm() {
    ...
    const role = useSelector(state => state && state.roleDetails && state.roleDetails.data)

    const initialValues = {
        id: role && role.id,
        name: role && role.name,
        description: role && role.description,
        status: role && role.isDisable
    }
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(updateRoleTodo({ 'values': values })).then((res) => update_res(res.payload, action))
        },
    })

    const update_res = (res, action) => {
        if (res && res.status == 200) {
            toast.success(lang.role + ' ' + lang.success_update, { position: "bottom-right" })
            setTimeout(() => {
                navigate('/role-list')
            }, 1500);
    ...
    useEffect(() => {
        ...
        dispatch(roleDetailsTodo({ 'id': state })).then((res) => role_res(res.payload))
    }, [])
