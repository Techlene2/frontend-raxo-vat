...
const { Text, Form, Button, useFormik, FormikProvider, FieldArray, SaveButton, AddCustomerSchema, TextArea, SelectStatus, SingleSelect, useDispatch, useSelector, useNavigate, toast, ToastContainer, Loader, FaPlus, FaTrash, Card, addCustomerTodo, gradeListTodo, customerTypeListTodo, departmentListTodo, taxTypeListTodo, currencyListTodo, customerGroupListTodo, regionListTodo, cityListTodo, customerCategoryListTodo, debounce, lang } = CustomerImport
...
export default function AddCustomerForm() {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(addCustomerTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.customer + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
                                        ...
                                        <>
                                            <div className="col-md-12">
                                                <Card className='mb-3'>
                                                    ...
                                                    <Card.Body>
                                                        {formik.values.person.map((filed, index) => (
                                                            // <div className="col-md-4">
                                        ...
                                        <>
                                            <div className="col-md-12">
                                                <Card className='mb-3'>
                                                    ...
                                                    <Card.Body>
                                                        <div className="row">
                                                            {formik.values.dispatch.map((filed, index) => (
