...
const { Text, Form, useFormik, SaveButton, ToastContainer, toast, SelectStatus, TextArea, useDispatch, addCustomerGroupTodo, useNavigate, AddCustomerGroupSchema, lang } = CustomerGroupImport
...
export default function AddCustomerGroupForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            dispatch(addCustomerGroupTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.customer_group + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
