...
const { Text, Form, SelectStatus, useFormik, SaveButton, AddCategorySchema, TextArea, addCategoryTodo, useDispatch, useNavigate, toast, ToastContainer, lang } = CategoryImport
...
export default function AddCategoryForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(addCategoryTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.category + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
