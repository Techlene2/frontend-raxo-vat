...
const { Form, Button, useFormik, Text, TextArea, SelectStatus, SaveButton, FormikProvider, FieldArray, FaPlus, FaTrash, AddRoutesSchema, ToastContainer, toast, useDispatch, useNavigate, addRoutesTodo, StandaloneSearchBox, lang } = RoutesImport

export default function AddRoutesForm() {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values, action) => {
            ...
            dispatch(addRoutesTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.routes + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
