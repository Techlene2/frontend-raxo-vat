...
const { Text, Form, useFormik, SaveButton, ToastContainer, toast, SelectStatus, TextArea, useDispatch, useNavigate, AddCustomerTypeSchema, addCustomerTypeTodo, lang } = CustomerTypeImport

export default function AddCustomerTypeForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            dispatch(addCustomerTypeTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.customer_type + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
