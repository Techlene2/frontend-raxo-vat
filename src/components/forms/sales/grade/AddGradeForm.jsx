...
const { Text, Form, useFormik, SaveButton, toast, ToastContainer, TextArea, useDispatch, useNavigate, addGradeTodo, AddGradeSchema, lang } = GradeImport
...
export default function AddGradeForm() {
  ...
  const { values, errors, touched, handleBlur, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: (values, action) => {
      dispatch(addGradeTodo({ 'values': values })).then((res) => add_res(res.payload, action))
    }
  })

  const add_res = (res, action) => {
    if (res && res.status == 201) {
      toast.success(lang.grade + ' ' + lang.success_add, { position: "bottom-right" })
      action.resetForm()
      setTimeout(() => {
        navigate('/grade-list')
      }, 1500);
