...
const { Text, Form, SelectStatus, useFormik, SaveButton, AddMeasureUnitTypeSchema, TextArea, useDispatch, useNavigate, addMeasureUnitTypeTodo, ToastContainer, toast, lang } = MeasureUnitTypeImport

export default function AddMeasureUnitTypeForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            console.log(values)
            dispatch(addMeasureUnitTypeTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        },
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.unit_type + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
