...
const { Text, Form, useFormik, SaveButton, ToastContainer, Loader, SelectStatus, BrandSchema, TextArea, addBrandTodo, useDispatch, useNavigate, toast, lang } = BrandImport
...
export default function AddBrandForm() {
    ...
    const { values, errors, touched, handleBlur, handleChange, handleSubmit, setFieldValue } = useFormik({
        ...
        onSubmit: (values, action) => {
            dispatch(addBrandTodo({ 'values': values })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.brand + ' ' + lang.success_add, { position: "bottom-right" })
            action.resetForm()
            setTimeout(() => {
                navigate('/brand-list')
            }, 1500);
