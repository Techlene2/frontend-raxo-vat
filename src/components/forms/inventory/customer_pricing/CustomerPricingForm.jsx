...
import { CustomerPricingImport } from './Imports'
const { Form, Button, Table, useFormik, FormikProvider, FieldArray, CustomerPricingSchema, SingleSelect, DatePicker, moment, Number, TextArea, SaveButton, useNavigate, useDispatch, useSelector, subCategoryListTodo, brandListTodo, Loader, customerPricingFilterTodo, Swal, ToastContainer, toast, updateCustomerPricingTodo, customerListTodo, debounce, lang } = CustomerPricingImport

export default function CustomerPricingForm() {

    const [loading, setLoading] = useState(true)
    const [pricingLoader, setPricingLoader] = useState(false)
    const [filterData, setFilterData] = useState({ "customer": '', "sub_cat": '', "brand": '', "effective_date": '' })
    ...
    const product_price = useSelector(state => state && state.customerPricingFilter && state.customerPricingFilter.data)
    ...
    const formik = useFormik({
        ...
        validationSchema: CustomerPricingSchema,
        enableReinitialize: true,

        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(updateCustomerPricingTodo({ 'values': values, 'filter': filterData })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.customer_pricing + ' ' + lang.success_update, { position: "bottom-right" })
            setTimeout(() => {
                navigate('/customer-pricing-list')
            }, 1500);
    ...
    const apply_filter = () => {
        if (formik.values.customer && formik.values.effective_date && formik.values.sub_cat || formik.values.customer && formik.values.effective_date && formik.values.brand) {
            setPricingLoader(true)
            setFilterData({
                "customer": formik.values.customer, "sub_cat": formik.values.sub_cat, "brand": formik.values.brand, "effective_date": formik.values.effective_date
            })
            dispatch(customerPricingFilterTodo({ "customer": formik.values.customer, 'date': formik.values.effective_date, 'sub_cat_id': formik.values.sub_cat, 'brand_id': formik.values.brand })).then((res) => pricing_res(res.payload))
        } else {
            Swal.fire({
                title: lang.customer_pricing_filter,
                text: lang.customer_pricing_filter_error,
                icon: "error",
    ...
    useEffect(() => {
        ...
        return () => {
            dispatch(customerPricingFilterTodo({ "customer": '', 'date': '', 'sub_cat_id': '', 'brand_id': '' }))
        }
    }, [])
