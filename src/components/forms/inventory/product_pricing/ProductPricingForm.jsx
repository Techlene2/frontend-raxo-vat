...
import { ProductPricingImport } from './Imports'
const { Form, Button, Table, useFormik, FormikProvider, FieldArray, ProductPricingSchema, SingleSelect, DatePicker, moment, Number, TextArea, SaveButton, useNavigate, useDispatch, useSelector, subCategoryListTodo, brandListTodo, Loader, productPricingFilterTodo, Swal, ToastContainer, toast, updateProductPricingTodo, lang } = ProductPricingImport

export default function ProductPricingForm() {

    const [loading, setLoading] = useState(true)
    const [pricingLoader, setPricingLoader] = useState(false)
    const [filterData, setFilterData] = useState({ "sub_cat": '', "brand": '', "effective_date": '' })
    ...
    const product_price = useSelector(state => state && state.productPricingFilter && state.productPricingFilter.data)
    ...
    const formik = useFormik({
        ...
        validationSchema: ProductPricingSchema,
        enableReinitialize: true,

        onSubmit: (values, action) => {
            // console.log(values)
            dispatch(updateProductPricingTodo({ 'values': values, 'filter': filterData })).then((res) => add_res(res.payload, action))
        }
    })

    const add_res = (res, action) => {
        if (res && res.status == 201) {
            toast.success(lang.product_pricing + ' ' + lang.success_update, { position: "bottom-right" })
            setTimeout(() => {
    ...
    const apply_filter = () => {
        if (formik.values.effective_date && formik.values.sub_cat || formik.values.effective_date && formik.values.brand) {
            setPricingLoader(true)
            setFilterData({
                "sub_cat": formik.values.sub_cat, "brand": formik.values.brand, "effective_date": formik.values.effective_date
            })
            dispatch(productPricingFilterTodo({ 'date': formik.values.effective_date, 'sub_cat_id': formik.values.sub_cat, 'brand_id': formik.values.brand })).then((res) => pricing_res(res.payload))
        } else {
            Swal.fire({
                title: lang.pricing_filter,
                text: lang.pricing_filter_error,
                icon: "error",
    ...
    useEffect(() => {
        ...
        return () => {
            dispatch(productPricingFilterTodo({ 'date': '', 'sub_cat_id': '', 'brand_id': '' }))
        }
    }, [])
